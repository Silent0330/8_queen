...
int main(int argc, char **argv)
{
	...
	for (int i = 0; i < board_size; i++) sta[i] = i;
	...
	while(true)
	{
		...
		if (k == -1)
		{
			cout << "Please input each row of queens in each column\n";
...
State RandomState()
{
	// Random state with queens in different columns
...
bool R_DLS(State node, int limit, vector<State> &explored, int &states_num, int &max_states_num)
{
	...
	node.Expand(successors, explored, false, true);
...
void UCS(State start)
{
	...
	while (discovered.size() > 0)
	{
		...
		cur.Expand(discovered, explored, false, true);
	}
}
